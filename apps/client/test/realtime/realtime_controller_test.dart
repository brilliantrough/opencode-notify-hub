...
void main() {
  ...
  test(
    'a throwing notification service is logged, not unhandled, '
    'and the event is still recorded in history',
    () async {
  ...
  group('realtimeControllerProvider', () {
    ...
    test('authenticated: controller starts with connect and a live subscription', () async {
    ...
    test(
      'background disconnects and foreground reconnects '
      '(appForegroundProvider gating)',
      () async {
        final c = buildContainer(_authenticated);
        expect(c.read(realtimeControllerProvider), isNotNull);
        expect(client.connectCalls, 1);
