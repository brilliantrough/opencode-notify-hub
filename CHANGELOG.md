...
- 在线入口和历史会话采用两行紧凑列表，关注/固定与打开靠右；窄屏图标按钮，历史发送/删除收进更多菜单。

### Fixed

- Plugin 到 OpenCode 的真实 HTTP 请求保留服务认证，修复 health 查询 401 导致版本 unknown；仅健康检查通过的入口上报 WebUI 可用性，未知版本可重新探测。
- `magic-context-` 内部后台会话在启动查询、事件和SDK补查中统一排除，避免临时根会话激活空目录入口并产生任务通知。
- 删除或归档后确认目录没有有效主会话就撤下入口，后续真实会话可重新注册；保留 idle 历史、显式远程目录与查询失败时的入口，并阻止过期快照/健康探测跨生命周期误操作。
- 客户端将未确认的 WebUI 能力显示为“WebUI 未确认”，不再误称“仅通知”。
