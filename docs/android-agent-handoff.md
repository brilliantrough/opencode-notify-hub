...
- 维护者已验收Windows紧凑首页并授权发布。Linux从统一最终SHA构建Android `0.2.0-beta.3+5`，沿用原签名；不复用beta.2验证APK。
- 使用锁文件对应镜像，先`flutter pub get --enforce-lockfile`，再`flutter build apk --release`（不加`--no-pub`）；核对版本码5、ABI、ZIP及证书。Android本轮真机验收仍未确认，发布说明如实披露。
- 本节优先于下文暂缓发布及补丁验证的历史记录。

## 当前修复：紧凑条目与空目录生命周期

- 共享首页在线入口/历史会话采用两行紧凑条目，操作右对齐；窄屏打开改为带提示图标，历史发送/删除在更多菜单中。Android与Windows/Linux使用相同Dart，无新增原生依赖。
- 320/600/1200px及双倍字号布局检查通过，Android本轮安装/真机验收待维护者进行；按协调端指定源码/补丁构建，不复用旧验证APK。
- Plugin新增内部任务过滤及删除/归档后空入口撤回，需另行替换OpenCode主机的JS并重启实际服务。客户端false能力标志显示“WebUI 未确认”。维护者已要求暂缓发布。
